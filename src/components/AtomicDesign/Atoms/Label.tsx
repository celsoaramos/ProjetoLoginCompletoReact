...
const Label: React.FC<LabelProps> = ({ text, textSize, fontWeight, color, htmlFor, className }) => {
  ...
  return (
    <label htmlFor={htmlFor} className={`${textSizeLabel} ${color} ${fontWeightLabel} ${className}`}>
      {text}
    </label>
  );
};
